...
/* Lexer */ function innerParse(filename, variable, rules, context) {
		...
		if ((this.lexeme().name() in firstTokens || Tokenizer.Token.LAMBDA.name() in firstTokens) &&
			...
			for(var j = 0; j < phrase.productions().length; j++) {
				...
				if (word in this.grammar().variables()) {
					...
					if (phrase.callback()) {
						phrase.callback()(phrase, word, lastLexeme.token(), lastLexeme.value(), context, true);
					}
				} else {
					...
					if (this.lexeme().name() != word && word != Tokenizer.Token.LAMBDA.name()) {
						// Throw the error
						throwError.call(this, filename, this.lexeme(), 'Expected %s found %s', word, this.lexeme().name());
					}
	...
	throwError.call(this, filename, this.lexeme(), 'Unmatched grammar token: %s', this.lexeme().token());
}
